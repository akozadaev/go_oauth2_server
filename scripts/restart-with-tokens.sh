...
# Скрипт для перезапуска с поддержкой токенов в PostgreSQL

set -e

echo "🔄 Перезапуск OAuth2 сервера с поддержкой токенов в PostgreSQL..."
...
# Проверка логов
echo ""
echo "📋 Последние логи OAuth2 Server:"
docker-compose logs --tail=20 oauth2-server
...
echo "   ./scripts/cleanup-tokens.sh - очистка токенов"
