...
Проект поддерживает горизонтальное масштабирование и может работать под высокой нагрузкой:

- ✅ **Stateless архитектура** - токены хранятся в PostgreSQL, JWT токены stateless
- ✅ **Несколько экземпляров** - можно запустить несколько экземпляров за балансировщиком
- ✅ **Connection pooling** - настроен пул соединений с БД (25 открытых, 5 idle)
- ✅ **Индексы БД** - оптимизированные запросы к базе данных

### Запуск с балансировщиком

```bash
# Запуск с масштабированием (3 экземпляра)
docker-compose --profile scalable -f docker-compose.scalable.yml up -d --scale oauth2-server=3

# Или используйте docker-compose scale
docker-compose --profile scalable -f docker-compose.scalable.yml up -d
docker-compose --profile scalable -f docker-compose.scalable.yml scale oauth2-server=3
```
